...
private:
    ...
    void respawn() {
        ...
        white->triggers.emplace_back([&](Rigidbody *const &rbody) {
...
public:
        ...
        if (!moving) {
            ...
            renderer->mouseHandle([&](const MouseMsg &msg) {
                if (!downed && msg.l_button_down) {
                    downed = true;
                    origin = Vec2f((float) msg.x, (float) msg.y);
                } else if (downed && msg.l_button) {
                    target = Vec2f((float) msg.x, (float) msg.y);
                ...
                } else origin = Vec2f((float) msg.x, (float) msg.y);
            });
        }
        moving = false;
    }
    ...
    void onRender() override {
        ...
        renderer->drawLine((int) start.x, (int) start.y, (int) end.x, (int) end.y, 0xFFFFFF);
    }
};
